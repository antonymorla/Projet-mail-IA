...
> **Erreur #2 — Oublier `largeur_hors_tout` ou `profondeur_hors_tout`** : les 3 sont indissociables.
> **Erreur #3 — Mettre le sur-mesure en `produits_complementaires`** : c'est un paramètre NATIF, PAS un produit.
>
...
> 2. ✅ `largeur_hors_tout="X.XX"` — dimension exacte en mètres (notation point, pas virgule)
...
> Si l'un est absent, le sur-mesure ne s'active pas correctement. **Vérifier les 3 avant chaque appel.**
>
...
1. **TOUJOURS tenter le configurateur WAPF en premier** (`generer_devis_terrasse_bois`). Le configurateur accepte **TOUTES les longueurs** disponibles pour chaque essence, y compris des longueurs intermédiaires qui ne sont PAS listées dans `rechercher_produits_detail`. **Ne JAMAIS décider de passer en fallback détail uniquement parce qu'une longueur n'apparaît pas dans l'API produits.** Le WAPF est la source de vérité pour les longueurs disponibles.

2. **TOUJOURS convertir en surface m²** — le configurateur WAPF travaille en m², PAS en nombre de lames :
   - **Formule** : `surface_m² = nb_lames × 0.145 × longueur_lame`
   ...
   - **Exemple** : client donne 21 lames de 2,75m → `quantite = ceil(21 × 0.145 × 2.75)` = **9 m²**. Mais le client dit "zone de 8m²" → utiliser **8** (la surface du client prime si cohérente)

3. **Fallback vers `generer_devis_terrasse_bois_detail` UNIQUEMENT si le configurateur WAPF échoue** (erreur retournée par l'outil, PAS parce qu'une longueur semble absente du catalogue) :
...
### ⛔ RÈGLE — LONGUEUR INDISPONIBLE AU DÉTAIL

> Quand une longueur demandée par le client n'est pas disponible au détail (ex : 1,85m n'existe pas), **toujours prendre la longueur supérieure la plus proche** (ex : 2,15m). Ne JAMAIS insister sur une longueur indisponible. Informer le client dans l'email que la longueur a été ajustée.

### ⛔ RÈGLE — NE PAS MÉLANGER CONFIGURATEUR ET DÉTAIL SUR LE MÊME DEVIS

> Si le commercial demande 2 devis (un configurateur + un au détail), faire **2 appels séparés** :
> - Devis 1 : `generer_devis_terrasse_bois` (configurateur, quantités conseillées)
> - Devis 2 : `generer_devis_terrasse_bois_detail` (quantités exactes du client)
> Ne pas mélanger les deux approches dans un même appel.
...
- Densité recommandée : **35 vis par m²** (pas 12 ni 20)
- Boîte standard : 200 vis
- **Formule** : `nb_boites = ceil(surface_m² × 35 / 200)`
- Exemple : 16m² → `ceil(16 × 35 / 200)` = **3 boîtes** (560 vis)
- Exemple : 34m² → `ceil(34 × 35 / 200)` = **6 boîtes** (1190 vis)
