...
>
> **`sur_mesure=True` est OBLIGATOIRE** dès qu'une dimension hors-tout est renseignée. Mais on n'est PAS obligé de remplir les 3 dimensions — seules celles qui diffèrent du standard sont nécessaires.
>
...
1. **TOUJOURS vérifier les longueurs en stock** via `rechercher_produits_detail(site="terrasse", recherche="[essence]")` — les longueurs dépendent du stock, elles ne sont PAS fixes et ne doivent JAMAIS être hardcodées. Puis tenter le configurateur WAPF en premier (`generer_devis_terrasse_bois`). **Ne JAMAIS décider de passer en fallback détail uniquement parce qu'une longueur n'apparaît pas dans l'API produits** — le configurateur peut accepter des longueurs supplémentaires.
